...
Begin VB.Form FunctionFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Fungsi Produk Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatButton BrowseUserID 
         ...
         _ExtentX        =   873
         _ExtentY        =   450
         MouseIcon       =   "FunctionFrm.frx":0000
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_fungsi where kodefungsi='" & sKodeUserAkses & "'"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnFunctionFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_fungsi order by kodefungsi asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_fungsi where kodefungsi >'" & Text1(0).text & "' order by kodefungsi asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_fungsi where kodefungsi<'" & Text1(0).text & "' order by kodefungsi desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_fungsi order by kodefungsi desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "Delete from master_fungsi where kodefungsi='" & skodefungsi & "'"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
    skodefungsi = ToText(Text1(0).text)
    snamafungsi = ToText(Text1(1).text)
     
    sUpdate = "update master_fungsi set "
    sUpdate = sUpdate & "namafungsi='" & snamafungsi & "' where "
    'sUpdate = sUpdate & " where "
    sUpdate = sUpdate & "kodefungsi='" & skodefungsi & "'"
    
    sInsert = "insert into master_fungsi ("
    sInsert = sInsert & "kodefungsi,namafungsi ) values "
    sInsert = sInsert & "("
    sInsert = sInsert & "'" & skodefungsi & "',"
    sInsert = sInsert & "'" & snamafungsi & "')"
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("kodefungsi")
    KodeUserAksesTemp = oRs("kodefungsi")
    Text1(0).Locked = True
    Text1(1).text = oRs("namafungsi")
