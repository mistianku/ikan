...
Begin VB.Form JenisFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Jenis Materi Kursus"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_jenis where kodejenis='" & sKodeUserAkses & "'"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnFeeFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_jenis order by kodejenis asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_jenis where kodejenis >'" & Text1(0).text & "' order by kodejenis asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_jenis where kodejenis<'" & Text1(0).text & "' order by kodejenis desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_jenis order by kodejenis desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
Dim ires As Integer
    ...
    If ires = 6 Then
        If DoSaveData Then
             ...
             MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnUserGroup
        End If
    End If
End Sub
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "Delete from master_jenis where kodejenis='" & skodejenis & "'"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
    skodejenis = ToText(Text1(0).text)
    snamajenis = ToText(Text1(1).text)
     
    sUpdate = "update master_jenis set "
    sUpdate = sUpdate & "namajenis='" & snamajenis & "' where "
    'sUpdate = sUpdate & " where "
    sUpdate = sUpdate & "kodejenis='" & skodejenis & "'"
    
    sInsert = "insert into master_jenis ("
    sInsert = sInsert & "kodejenis,namajenis ) values "
    sInsert = sInsert & "("
    sInsert = sInsert & "'" & skodejenis & "',"
    sInsert = sInsert & "'" & snamajenis & "')"
...
oBrowse.ShowFinder BrowsJenisMateri, "", ubAscending, DBaseConection.Modul
...
sTitle = "Master Jenis Materi Kursus"
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("kodejenis")
    KodeUserAksesTemp = oRs("kodejenis")
    Text1(0).Locked = True
    Text1(1).text = oRs("namajenis")
