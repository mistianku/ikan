...
Begin VB.Form MonitoringPelunasanFaktur 
   BackColor       =   &H8000000A&
   Caption         =   "Monitoring Entry Tukar Faktur Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Label Label2 
         Caption         =   "Total Bayar Nilai Faktur"
         Height          =   285
         Left            =   1200
         TabIndex        =   27
         Top             =   720
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSFlex8LCtl.VSFlexGrid oGrid1 
         ...
         FormatString    =   $"MonitoringPelunasanFaktur.frx":0000
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatComboBox FlatComboBox1 
         ...
         EndProperty
         MousePointer    =   0
         MouseIcon       =   "MonitoringPelunasanFaktur.frx":016F
      End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatButton FlatButton1 
         ...
         _ExtentX        =   3413
         _ExtentY        =   661
         MouseIcon       =   "MonitoringPelunasanFaktur.frx":018B
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "MonitoringPelunasanFaktur"
...
Dim stotallunas, stotalblmlunas, stotalnilaifaktur, snilaibayarnilaifaktur As Double
...
Dim ires As Integer
    ...
    If ires = 6 Then
        If DoSaveData Then
             ...
             MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnMonitoringTukarFaktur
        End If
    End If
End Sub
...
If MsgBox("Set Default Tempo Pembayaran ", vbYesNo) = vbYes Then
    UpdateSetDefaultJTempo
    MsgBox "Set Defoult Tempo Pembayaran Sudah di Update !!", vbInformation
End If
End Sub
...
sTitle = "Monitoring Pelunasan Faktur"
...
cleardata
istatus = SettingForm
FlatComboBox1.AddItem "No.Tukar.Faktur"
...
DatePicker1(1).value = Year(Now()) & "-" & Month(Now()) & "-01"
Text1(0).Locked = False
ShowTempoBayar (MenuFrm.sjtempo)
sawal = 1
'ShowGrid
End Sub
...
'            .TextMatrix(.row, 5) = DatePicker1(1).value + Val(Text1(1))
'        Else
'            .TextMatrix(.row, 2) = ""
'            .TextMatrix(.row, 5) = ""
'        End If
...
stukarfaktur = IIf(Option1(0).value, "0", IIf(Option1(1).value, "1", "A"))
...
'skeytgl INT ,stgl DATE
    ...
    sQuery = "CALL sp_transaksi_keluar_lunas_faktur_get('"
...
'        .COLWIDTH(2) = .Width - (.COLWIDTH(0) + .COLWIDTH(1) + .COLWIDTH(5) + 100)
        ...
        If Not oRsDetail.EOF Then
            ...
            Do While Not oRsDetail.EOF
                ...
                .TextMatrix(i, 0) = 0
                .TextMatrix(i, 1) = IIf(oRsDetail("lunas_sts") = "0", 0, 1)
                .TextMatrix(i, 2) = IIf(IsNull(oRsDetail("kwitansino")), "", oRsDetail("kwitansino"))
                ...
                If oRsDetail("lunas_sts") = "0" Then
                    stotallunas = stotalnilaifaktur + ToNumber(oRsDetail("totalsetppn"))
                Else
                    stotalblmlunas = stotalblmlunas + ToNumber(oRsDetail("totalsetppn"))
                End If
                stotalnilaifaktur = stotalnilaifaktur + ToNumber(oRsDetail("totalsetppn"))
                oRsDetail.MoveNext
            Loop
            .Select 1, 0
...
On Error GoTo errhandler
    ...
    Dim stukarfaktur As String
    Dim stgltukarfaktur, stgljtfaktur As String
    ...
    With oGrid1(0)
        For irow = 1 To .Rows - 1
            stukarfaktur = IIf(.TextMatrix(irow, 1) = -1, "1", "0")
            stgltukarfaktur = ToText(.TextMatrix(irow, 2))
            stgljtfaktur = ToText(.TextMatrix(irow, 5))
            snodokumen = .TextMatrix(irow, 3)
            sQuery = "call sp_transaksi_keluar_lunas_faktur_update('" & snodokumen & "','"
            sQuery = sQuery & stukarfaktur & "','"
...
Public Sub ShowTempoBayar(sDays As Integer)
Text1(1).Locked = True
Select Case sDays
Case 14
...
Public Sub UpdateSetDefaultJTempo()
On Error GoTo errhandler
    Dim oKon As New ADODB.Connection
    Dim oRsDetail As New ADODB.Recordset
    Dim sCekDataDetail1 As Integer
    ...
    MenuFrm.sjtempo = Text1(1)
    If oKon.State = 1 Then oKon.Close
    oKon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Update master_preferences set jtempo='" & Text1(1) & "' where id=1"
    oKon.Execute (sQuery)
    oKon.Close
    Exit Sub
errhandler:
...
Public Sub oRecalculateBayar()
On Error GoTo errhandler
    Dim stotalbayarQ, irow As Double
    snilaibayarnilaifaktur = 0
    With oGrid1(0)
        For irow = 1 To .Rows - 1
            If .TextMatrix(irow, 1) = -1 Then
                snilaibayarnilaifaktur = snilaibayarnilaifaktur + ToNumber(.TextMatrix(irow, 8))
            End If
        Next
        
    End With
Text1(2).text = formatRupiah(snilaibayarnilaifaktur)
    Exit Sub
errhandler:
    MainModule.ShowMessage Err.Description, "Save Produk Diskon"
End Sub
