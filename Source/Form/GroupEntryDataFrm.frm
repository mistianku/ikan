...
Begin VB.Form GroupEntryDataFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Group Entry Data Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatButton BrowseUserID 
         ...
         _ExtentX        =   873
         _ExtentY        =   450
         MouseIcon       =   "GroupEntryDataFrm.frx":0000
   ...
   End
End
Attribute VB_Name = "GroupEntryDataFrm"
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from tblgrupdataentry where gr_dataentry='" & sKodeUserAkses & "'"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnGroupEntryDataFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from tblgrupdataentry order by gr_dataentry asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from tblgrupdataentry where gr_dataentry >'" & Text1(0).text & "' order by gr_dataentry asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from tblgrupdataentry where gr_dataentry<'" & Text1(0).text & "' order by gr_dataentry desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from tblgrupdataentry order by gr_dataentry desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "Delete from tblgrupdataentry where gr_dataentry='" & sgr_dataentry & "'"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
    sgr_dataentry = ToText(Text1(0).text)
    snm_grupdata = ToText(Text1(1).text)
     
    sUpdate = "update tblgrupdataentry set "
    sUpdate = sUpdate & "nm_grupdata='" & snm_grupdata & "' where "
    'sUpdate = sUpdate & " where "
    sUpdate = sUpdate & "gr_dataentry='" & sgr_dataentry & "'"
    
    sInsert = "insert into tblgrupdataentry ("
    sInsert = sInsert & "gr_dataentry,nm_grupdata ) values "
    sInsert = sInsert & "("
    sInsert = sInsert & "'" & sgr_dataentry & "',"
    sInsert = sInsert & "'" & snm_grupdata & "')"
...
sTitle = "Master Group Entry Data"
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("gr_dataentry")
    KodeUserAksesTemp = oRs("gr_dataentry")
    Text1(0).Locked = True
    Text1(1).text = oRs("nm_grupdata")
