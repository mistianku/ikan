...
Begin VB.Form FeeFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Fee Produk Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_fee where kodediskon='" & sKodeUserAkses & "'"
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_fee order by kodediskon asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_fee where kodediskon >'" & Text1(0).text & "' order by kodediskon asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_fee where kodediskon<'" & Text1(0).text & "' order by kodediskon desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_fee order by kodediskon desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "call sp_delete_master_fee('" & skodediskon & "')"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
Dim scall As String
    ...
    sdiskon = ToNumber(Text1(2).text)
     
    scall = "('" & skodediskon & "','"
    scall = scall & sketerangan & "','"
    scall = scall & toNumberIndonesia(Text1(2).text) & "','"
    scall = scall & MenuFrm.sUserID & "','"
    scall = scall & Format(Now(), "YYYY-MM-DD") & "')"
    
    sUpdate = "call sp_update_master_fee" & scall
    sInsert = "call sp_insert_master_fee" & scall

'    sUpdate = "update master_fee set "
...
'
'    sInsert = "insert into master_fee ("
