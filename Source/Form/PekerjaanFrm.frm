...
Begin VB.Form PekerjaanFrm 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatButton BrowseUserID 
         ...
         _ExtentX        =   873
         _ExtentY        =   450
         MouseIcon       =   "PekerjaanFrm.frx":0000
   ...
   End
End
Attribute VB_Name = "PekerjaanFrm"
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_pekerjaan where kode='" & sKodeUserAkses & "'"
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.parkir)
    sQuery = "Select *  from master_pekerjaan order by kode asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.parkir)
    sQuery = "Select  *  from master_pekerjaan where kode >'" & Text1(0).Text & "' order by kode asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.parkir)
    sQuery = "Select  *  from master_pekerjaan where kode<'" & Text1(0).Text & "' order by kode desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.parkir)
    sQuery = "Select *  from master_pekerjaan order by kode desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.parkir)
        sQuery = "Delete from master_pekerjaan where kode='" & skode & "'"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
    ...
    spekerjaan = ToText(Text1(1).Text)
     
    sUpdate = "update master_pekerjaan set "
    sUpdate = sUpdate & "pekerjaan='" & spekerjaan & "' where "
    'sUpdate = sUpdate & " where "
    sUpdate = sUpdate & "kode='" & skode & "'"
    
    sInsert = "insert into master_pekerjaan ("
    sInsert = sInsert & "kode,pekerjaan ) values "
    sInsert = sInsert & "("
    sInsert = sInsert & "'" & skode & "',"
    sInsert = sInsert & "'" & spekerjaan & "')"
...
sTitle = "Master Pekerjaan"
'lblTitle.Caption = "  : .  " & sTitle & "  . :  "
Me.Caption = " " & sTitle & " "
MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnPekerjaan
