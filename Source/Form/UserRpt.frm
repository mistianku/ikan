...
Begin VB.Form UserRpt 
   BackColor       =   &H8000000A&
   Caption         =   "Master Pengguna Report"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
...
Case 0
    ...
    oBrowse.ShowFinder BrowsUser, "", ubAscending, DBaseConection.Modul
...
skodebrandfr = IIf(Text1(0) = "", oFindByQuery("select UserID from MASTER_USER order by UserID asc limit 1", DBaseConection.login), Text1(0))
skodebrandto = IIf(Text1(1) = "", oFindByQuery("select UserID from MASTER_USER order by UserID desc limit 1", DBaseConection.login), Text1(1))

Dim sKriteria As String

sKriteria = " where UserID  between '" & skodebrandfr & "' and '" & skodebrandto & "'"

sQuery = "SELECT  UserID, NamaUser, mu.kodegroup  ,groupuser "
sQuery = sQuery & "FROM MASTER_USER mu INNER JOIN  master_group_user mgu ON mu.kodegroup=mgu.kodegroup    "
...
With arMasterDiskonProduk
    ...
    .Label24.Caption = "Master User"
    .lblPeriode.Caption = "Kode User : " & skodebrandfr & " s/d  " & skodebrandto
    .lblPeriode2.Visible = False

    .adoKu.Provider = "MSDASQL.1"
    .adoKu.ConnectionString = MainModule.Conectionku(DBaseConection.login)
    .adoKu.Source = sQuery
    .lblKode = "Kode User"
    .lblketerangan = "Nama User "
    .txtkode.DataField = "UserID"
    .txtketerangan.DataField = "NamaUser"
    
    .lbloption1 = "Kode Group"
    .lbloption2 = "Keterangan"
    .txtoption1.OutputFormat = ""
    .txtoption1.DataField = "kodegroup"
    .txtoption2.DataField = "groupuser"
