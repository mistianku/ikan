...
Begin VB.Form BrandFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Brand Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_brand where kodebrand='" & sKodeUserAkses & "'"
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_brand order by kodebrand asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_brand where kodebrand >'" & Text1(0).text & "' order by kodebrand asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_brand where kodebrand<'" & Text1(0).text & "' order by kodebrand desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_brand order by kodebrand desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("kodebrand")
    KodeUserAksesTemp = oRs("kodebrand")
    Text1(0).Locked = True
    Text1(1).text = oRs("namabrand")
