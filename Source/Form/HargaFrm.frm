...
Begin VB.Form HargaFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Harga Produk Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
...
Dim snamaharga As String
Dim skelipatan As String
Dim sbaseharga As String
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_harga where kodeharga='" & sKodeUserAkses & "'"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnHargaFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_harga order by kodeharga asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_harga where kodeharga >'" & Text1(0).text & "' order by kodeharga asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_harga where kodeharga<'" & Text1(0).text & "' order by kodeharga desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_harga order by kodeharga desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "call sp_delete_master_harga ('" & skodeharga & "')"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
Dim scall As String
    skodeharga = ToText(Text1(0).text)
    snamaharga = ToText(Text1(1).text)
    skelipatan = (toNumberIndonesia(Text1(2).text))
    sbaseharga = ToText(IIf(Text1(3).text = "", Text1(0).text, Text1(3).text))
    sauditdate = Format(Now(), "YYYY-MM-DD")
    saudituser = MenuFrm.sUserID
    
        scall = "('" & skodeharga & "','"
        scall = scall & snamaharga & "','"
        scall = scall & skelipatan & "','"
        scall = scall & sbaseharga & "','"
        scall = scall & saudituser & "','"
        scall = scall & sauditdate & "')"
        
        sUpdate = "call sp_update_master_harga " & scall
        sInsert = "call sp_insert_master_harga " & scall
    
'    sUpdate = "update master_harga set "
'    sUpdate = sUpdate & "namaharga='" & snamaharga & "' , "
'     sUpdate = sUpdate & "kelipatan='" & skelipatan & "' , "
'    sUpdate = sUpdate & "baseharga='" & sbaseharga & "' where "
'    'sUpdate = sUpdate & " where "
'    sUpdate = sUpdate & "kodeharga='" & skodeharga & "'"
'
'    sInsert = "insert into master_harga ("
'    sInsert = sInsert & "kodeharga,namaharga,kelipatan,baseharga ) values "
'    sInsert = sInsert & "("
'    sInsert = sInsert & "'" & skodeharga & "',"
'    sInsert = sInsert & "'" & snamaharga & "',"
'    sInsert = sInsert & "'" & skelipatan & "',"
'    sInsert = sInsert & "'" & sbaseharga & "')"
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("kodeharga")
    KodeUserAksesTemp = oRs("kodeharga")
    Text1(0).Locked = True
    Text1(1).text = ToText(oRs("namaharga"))
    Text1(2).text = FormatNumber(oRs("kelipatan"), 2)
    Text1(3).text = ToText(oRs("baseharga"))
