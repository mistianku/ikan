...
Begin VB.Form CategoryFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Kategori Produk Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatButton BrowseUserID 
         ...
         _ExtentX        =   873
         _ExtentY        =   450
         MouseIcon       =   "CategoryFrm.frx":0000
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_kategori where kodekategori='" & sKodeUserAkses & "'"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnCategoryFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_kategori order by kodekategori asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_kategori where kodekategori >'" & Text1(0).text & "' order by kodekategori asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_kategori where kodekategori<'" & Text1(0).text & "' order by kodekategori desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_kategori order by kodekategori desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "Delete from master_kategori where kodekategori='" & skodekategori & "'"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
    skodekategori = ToText(Text1(0).text)
    snamakategori = ToText(Text1(1).text)
     
    sUpdate = "update master_kategori set "
    sUpdate = sUpdate & "namakategori='" & snamakategori & "' where "
    'sUpdate = sUpdate & " where "
    sUpdate = sUpdate & "kodekategori='" & skodekategori & "'"
    
    sInsert = "insert into master_kategori ("
    sInsert = sInsert & "kodekategori,namakategori ) values "
    sInsert = sInsert & "("
    sInsert = sInsert & "'" & skodekategori & "',"
    sInsert = sInsert & "'" & snamakategori & "')"
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("kodekategori")
    KodeUserAksesTemp = oRs("kodekategori")
    Text1(0).Locked = True
    Text1(1).text = oRs("namakategori")
