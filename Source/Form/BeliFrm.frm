...
Begin VB.Form BeliFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Transaksi Pembelian Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSFlex8LCtl.VSFlexGrid ogrid 
         ...
         FormatString    =   $"BeliFrm.frx":12A83
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
...
Dim sharga_current, sharga_max As Double
...
On Error GoTo errhandler
    ...
    sQuery = " call sp_transaksi_masuk_get('" & sKodeUserAkses & "',0)"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnBeliFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "call sp_transaksi_masuk_get('" & Text1(0).text & "',1)"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "call sp_transaksi_masuk_get('" & Text1(0).text & "',3)"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "call sp_transaksi_masuk_get('" & Text1(0).text & "',2)"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "call sp_transaksi_masuk_get('" & Text1(0).text & "',4)"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        ...
        If istatus = DataBaru Then
            sdocentry = oFindByQuery("select docentry from transaksi_masuk where nodokumen='" & Text1(0) & "'", DBaseConection.Modul)
        End If
        SaveGrid sdocentry
        oCon.Close
        DoSaveData = True
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "Delete from transaksi_masuk where nodokumen='" & snodokumen & "'"
        oCon.Execute sQuery
        oCon.Close
...
Public Sub NewData()
    If MenuFrm.sAplikasiDemo Then
        If oCekJumlahTrx("transaksi_masuk", MenuFrm.sMaxIsiTable) Then Exit Sub
    End If
...
On Error GoTo errhandler
    ...
    If istatus = StatusForm.DataBaru Then
        snodokumen = IIf(Text1(0).text = "", GetDocnum(transaksi_pembelian, True, DBaseConection.Modul), Text1(0).text)
    ...
    sQuery = "call sp_transaksi_masuk_update('" & sdocentry & "','"
...
Select Case Index
Case 0
oBrowse.ShowFinder BrowsPembelian, "", ubDescending, DBaseConection.Modul
...
Case 5
    ...
    If Not oBrowse.YangDipilih = "" Then
        sharga_max = oFindByQuery("select harga from master_produk_harga where kodeproduk='" & oBrowse.YangDipilih & "' and kodeharga='" & Text1(6) & "' ", DBaseConection.Modul)
        sharga_current = oFindByQuery("select harga from master_produk_harga_supplier where kodeproduk='" & oBrowse.YangDipilih & "' and kodeharga='" & Text1(6) & "' and kodecustomer='" & Text1(1) & "'", DBaseConection.Modul)
             ...
             With ogrid
              ...
              .TextMatrix(.row, 3) = oFindByQuery("select harga from master_produk_harga_supplier where kodeproduk='" & .TextMatrix(.row, 0) & "' and kodeharga='" & Text1(6) & "' and kodecustomer='" & Text1(1) & "'", DBaseConection.Modul)
              .TextMatrix(.row, 4) = 0 'oFindByQuery("select fee from master_produk_harga_customer where kodecustomer='" & Text1(1) & "' and kodeproduk='" & .TextMatrix(.row, 0) & "' and kodeharga='" & Text1(6) & "'", DBaseConection.Modul)
            ...
            If slock_max_price = "1" Then
                If sharga_current > sharga_max Then
                     MsgBox "Harga Melebihi Batas Harga Maksimal ( " & formatRupiah(sharga_max) & " ) , Transaksi Dibatalkan !!", vbInformation
                     Undo
                End If
            End If
...
sTitle = "Transaksi Pembelian"
'lblTitle.Caption = "  : .  " & sTitle & "  . :  "
Me.Caption = " " & sTitle & " "
MenuFrm.SetToolbarku Me, istatus, MenuFrm.sUserID, mnBeliFrm
...
slock_max_price = oFindByQuery("SELECT lock_max_price FROM master_preferences", DBaseConection.Modul)
skodeharga_maxprice = oFindByQuery("SELECT kodeharga_maxprice FROM master_preferences", DBaseConection.Modul)
...
Case 0
    ...
    Else
         ...
         .TextMatrix(.row, 3) = oFindByQuery("select harga from master_produk_harga_supplier where kodeproduk='" & sKodeQ & "' and kodeharga='" & Text1(6) & "' and kodecustomer='" & Text1(1) & "'", DBaseConection.Modul)
         .TextMatrix(.row, 4) = oFindByQuery("select fee from master_produk_harga_supplier where kodeproduk='" & sKodeQ & "' and kodeharga='" & Text1(6) & "' and kodecustomer='" & Text1(1) & "'", DBaseConection.Modul)
    ...
    '         .TextMatrix(.row, 1) = oFindByQuery("select namaproduk from master_produk where kodeproduk='" & .TextMatrix(.row, 0) & "'", DBaseConection.Modul)
    '         .TextMatrix(.row, 3) = oFindByQuery("select harga from master_produk_harga_supplier where kodeproduk='" & .TextMatrix(.row, 0) & "' and kodeharga='" & Text1(6) & "' and kodecustomer='" & Text1(1) & "'", DBaseConection.Modul)
    '         .TextMatrix(.row, 4) = oFindByQuery("select fee from master_produk_harga_supplier where kodeproduk='" & .TextMatrix(.row, 0) & "' and kodeharga='" & Text1(6) & "' and kodecustomer='" & Text1(1) & "'", DBaseConection.Modul)
    '         .TextMatrix(.row, 11) = oFindByQuery("select diskon from master_produk_diskon where kodeproduk='" & .TextMatrix(.row, 0) & "' and kodediskon='" & Text1(5) & "'", DBaseConection.Modul)
    '         .TextMatrix(.row, 6) = ToNumber(.TextMatrix(.row, 3)) * (ToNumber(.TextMatrix(.row, 11)) / 100)
    '       .Select .row, 2
    '       .EditCell
...
On Error GoTo errhandler
    ...
    sQuery = "CALL sp_transaksi_masuk_detail1_get('" & sdocentry & "')"
...
On Error GoTo errhandler
    ...
    If oKon.State = 1 Then oKon.Close
    oKon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "call sp_master_supplier_mov('"
    ...
    If Not oRsDetail.EOF Then
        ...
        Text1(17) = ToText(oRsDetail("kodesalesman"))
...
On Error GoTo errhandler
    ...
    With ogrid
            ...
            For i = 1 To .Rows - 1
                ...
                sQuery = "Call sp_transaksi_masuk_detail1_insert('" & sdocentry & "','"
    ...
    With ogrid
            ...
            For i = 1 To .Rows - 1
                ...
                slinenum1 = ToNumber(.TextMatrix(i, .Cols - 2))
                        ...
                        oKon.Execute "delete from transaksi_masuk_detail1 where docentry='" & sdocentry & "' and linenum='" & slinenum1 & "'"
...
sQuery = "call sp_transaksi_masuk_form('"
...
Exit Sub
errhandler:
MainModule.ShowMessage Err.Description, "Transaksi Pembelian"
End Sub
