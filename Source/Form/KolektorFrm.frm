...
Begin VB.Form KolektorFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Kolektor Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatButton BrowseUserID 
         ...
         _ExtentX        =   873
         _ExtentY        =   450
         MouseIcon       =   "KolektorFrm.frx":0000
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_kolektor where kodekolektor='" & sKodeUserAkses & "'"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnKolektorFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_kolektor order by kodekolektor asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_kolektor where kodekolektor >'" & Text1(0).text & "' order by kodekolektor asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_kolektor where kodekolektor<'" & Text1(0).text & "' order by kodekolektor desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_kolektor order by kodekolektor desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "Delete from master_kolektor where kodekolektor='" & skodekolektor & "'"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
    skodekolektor = ToText(Text1(0).text)
    snamakolektor = ToText(Text1(1).text)
     
    sUpdate = "update master_kolektor set "
    sUpdate = sUpdate & "namakolektor='" & snamakolektor & "' where "
    'sUpdate = sUpdate & " where "
    sUpdate = sUpdate & "kodekolektor='" & skodekolektor & "'"
    
    sInsert = "insert into master_kolektor ("
    sInsert = sInsert & "kodekolektor,namakolektor ) values "
    sInsert = sInsert & "("
    sInsert = sInsert & "'" & skodekolektor & "',"
    sInsert = sInsert & "'" & snamakolektor & "')"
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("kodekolektor")
    KodeUserAksesTemp = oRs("kodekolektor")
    Text1(0).Locked = True
    Text1(1).text = oRs("namakolektor")
