...
Begin VB.Form MonitoringKwitansiBrowseTF 
   BackColor       =   &H8000000A&
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatButton FlatButton1 
         ...
         _ExtentX        =   3413
         _ExtentY        =   661
         MouseIcon       =   "MonitoringKwitansiBrowseTF.frx":0000
      ...
      Begin VSDFLATS.FlatButton FlatButton1 
         ...
         _ExtentX        =   3413
         _ExtentY        =   661
         MouseIcon       =   "MonitoringKwitansiBrowseTF.frx":001C
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
      ...
      Begin VSDFLATS.FlatButton FlatButton1 
         ...
         _ExtentX        =   3413
         _ExtentY        =   661
         MouseIcon       =   "MonitoringKwitansiBrowseTF.frx":0038
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSFlex8LCtl.VSFlexGrid oGrid1 
         ...
         FormatString    =   $"MonitoringKwitansiBrowseTF.frx":0054
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VSDFLATS.FlatComboBox FlatComboBox1 
         ...
         EndProperty
         MousePointer    =   0
         MouseIcon       =   "MonitoringKwitansiBrowseTF.frx":01C5
         Enabled         =   0   'False
      End
   ...
   End
End
Attribute VB_Name = "MonitoringKwitansiBrowseTF"
...
stukarfaktur = IIf(Option1(0).value, "1", IIf(Option1(1).value, "0", "A"))
sskeyfind = 2 ' FlatComboBox1.ListIndex + 1
skata = "" & Text1(0) & ""
skeytgl = IIf(Option2(0).value, 1, IIf(Option2(1).value, 12, 3))
...
'skeytgl INT ,stgl DATE
    ...
    sQuery = "CALL sp_transaksi_kwitansi_get_lhppTF('"
...
'                .TextMatrix(i, 2) = IIf(IsNull(oRsDetail("tgltukarfaktur")), "",dbaseConection.Modul, oRsDetail("tgltukarfaktur"))
                .TextMatrix(i, 1) = RTrim(oRsDetail("nodokumen"))
                .TextMatrix(i, 2) = RTrim(oRsDetail("tgldokumen"))
'                .TextMatrix(i, 5) = IIf(IsNull(oRsDetail("tgljtfaktur")), "",dbaseConection.Modul, oRsDetail("tgljtfaktur"))
                .TextMatrix(i, 3) = RTrim(oRsDetail("kodecustomer"))
                .TextMatrix(i, 4) = RTrim(oRsDetail("namacustomer"))
                .TextMatrix(i, 5) = RTrim(oRsDetail("totalsetppn"))
                .TextMatrix(i, 6) = RTrim(oRsDetail("totalbyrkwitansi"))
                .TextMatrix(i, 7) = RTrim(oRsDetail("sisanilkwitansi"))
               ' .TextMatrix(i, .Cols - 3) = RTrim(oRsDetail("batchid"))
...
End Sub
Public Sub ShowForm(ogrid As VSFlexGrid, sbatchid As Integer, sdocentry As Double, skodecustomer As String)
    Set oGridasal = ogrid
    sbatchidasal = sbatchid
...
With oGrid1(0)
    ...
    If .TextMatrix(i, 0) = -1 Then
        ...
        oGridasal.TextMatrix(oGridasal.Rows - 1, 0) = .TextMatrix(i, 1)
        oGridasal.TextMatrix(oGridasal.Rows - 1, 1) = .TextMatrix(i, 2)
        oGridasal.TextMatrix(oGridasal.Rows - 1, 2) = .TextMatrix(i, 3)
        oGridasal.TextMatrix(oGridasal.Rows - 1, 3) = .TextMatrix(i, 4)
        oGridasal.TextMatrix(oGridasal.Rows - 1, 4) = .TextMatrix(i, 5)
        'oGridasal.TextMatrix(oGridasal.Rows - 1, 5) = .TextMatrix(i, 6)
        oGridasal.TextMatrix(oGridasal.Rows - 1, 5) = .TextMatrix(i, 7)
        oGridasal.TextMatrix(oGridasal.Rows - 1, oGridasal.Cols - 4) = sbatchidasal
        oGridasal.TextMatrix(oGridasal.Rows - 1, oGridasal.Cols - 3) = sdocentryasal
        oGridasal.TextMatrix(oGridasal.Rows - 1, oGridasal.Cols - 2) = 0
