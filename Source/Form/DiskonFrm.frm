...
Begin VB.Form DiskonFrm 
   BackColor       =   &H8000000A&
   Caption         =   "Master Diskon Produk Form"
   ClientHeight    =   5835
   ClientLeft      =   -135
   ClientTop       =   645
   ClientWidth     =   10170
...
On Error GoTo errhandler
    ...
    sQuery = "Select * from master_diskon where kodediskon='" & sKodeUserAkses & "'"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
        istatus = Normal
        MenuFrm.SetToolbarku Me, istatus, MenuFrm.sGroupUserID, mnDiskonFrm
    End If
    oCon.Close
    Exit Sub
errhandler:
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_diskon order by kodediskon asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
    oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_diskon where kodediskon >'" & Text1(0).text & "' order by kodediskon asc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select  *  from master_diskon where kodediskon<'" & Text1(0).text & "' order by kodediskon desc limit 1"
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    End If
...
On Error GoTo errhandler
    If oCon.State = 1 Then oCon.Close
     oCon.Open MainModule.Conectionku(DBaseConection.Modul)
    sQuery = "Select *  from master_diskon order by kodediskon desc limit 1 "
    Set oRs = oCon.Execute(sQuery)
    If Not oRs.EOF Then
        showData
    Else
...
On Error GoTo errhandler
    If setData Then
        If oCon.State = 1 Then oCon.Close
         oCon.Open MainModule.Conectionku(DBaseConection.Modul)
        sQuery = "call sp_delete_master_diskon('" & skodediskon & "')"
        oCon.Execute sQuery
        oCon.Close
        DoDeleteData = True
        istatus = Normal
...
On Error GoTo errhandler
Dim scall As String
    skodediskon = ToText(Text1(0).text)
    sketerangan = ToText(Text1(1).text)
    sdiskon = (toNumberIndonesia((Text1(2).text)))
     
    scall = "('" & skodediskon & "','"
    scall = scall & sketerangan & "','"
    scall = scall & sdiskon & "','"
    scall = scall & MenuFrm.sUserID & "','"
    scall = scall & Format(Now(), "YYYY-MM-DD") & "')"
    
    sUpdate = "call sp_update_master_diskon" & scall
    sInsert = "call sp_insert_master_diskon" & scall

'    sUpdate = "update master_diskon set "
'    sUpdate = sUpdate & "keterangan='" & sketerangan & "' , "
'    sUpdate = sUpdate & "diskon='" & sdiskon & "' where "
'    'sUpdate = sUpdate & " where "
'    sUpdate = sUpdate & "kodediskon='" & skodediskon & "'"
'
'    sInsert = "insert into master_diskon ("
'    sInsert = sInsert & "kodediskon,keterangan,diskon ) values "
'    sInsert = sInsert & "("
'    sInsert = sInsert & "'" & skodediskon & "',"
'    sInsert = sInsert & "'" & sketerangan & "','" & sdiskon & "')"
...
On Error GoTo errhandler
    cleardata
    Text1(0).text = oRs("kodediskon")
    KodeUserAksesTemp = oRs("kodediskon")
    Text1(0).Locked = True
    Text1(1).text = oRs("keterangan")
    Text1(2).text = FormatNumber(oRs("diskon"))
